...
CREATE OR REPLACE FUNCTION user_group_membership_update() RETURNS TRIGGER AS $$
    DECLARE
    row RECORD;
    output TEXT;
    ...
    PERFORM pg_notify('user_group_membership_update', output);
...
CREATE OR REPLACE TRIGGER trigger_user_group_membership_update
  AFTER INSERT OR UPDATE OR DELETE
  ON user_group_membership
  FOR EACH ROW
  EXECUTE PROCEDURE user_group_membership_update();